...
# input: hg38 GTF file to locate gene TSS on the human genome, RNA-seq data, cooler of specified resolution
...
def fetch_gene_ensembl_api(ensembl_ids, batch_size=1000):
    # add 'chr' in front of chrom number for convention standa'rd whenever using ensemble
    """
...
def fetch_gene_gtf(gtf_file, ensembl_ids):
...
def process_rna_seq(infile, outfile, gtf_file, normalisation="log1p_t"):
...
def assign_tss_to_hic_bin_id(infile, res0, bins_3D):
    ...
    tss_bin_res = f"TSSBin_{res0}"  # header of infile containing bin info of the TSS

    # Make gene_bins df
    gene_bins = gene_list[["gene_name", tss_bin_res]].copy()
    # Split 'bin_res' into 'chrom' and 'start', ensuring correct assignment
    gene_bins[["chrom", "start"]] = gene_bins[tss_bin_res].str.split(":", expand=True)
    gene_bins["start"] = (
