...
from utils import image_normalize, distance_normalize, calculate_expected
...
def trainRF(trainset, labels_, nproc=4):
    ...
    clf = GridSearchCV(forest(), param_grid=params,
                       scoring=mcc, verbose=2, n_jobs=nproc, cv=5) #scoring: model performance evaluation (eg: precision + recall)
...
def learn_distri_kde(coords, res):
    ...
    counts, bins = np.histogram(dis, bins=100) #sorted based on 1-d genomic distance b/w loops binned into 100 categories
    long_end = max(int(bins[-1]), 5000000//res) #max distance between loop anchors either the max distance in the loop anchor data OR 5Mb
    # wherever the first increase in the histogram is observed, long range interactions start from there 
    # there's a decay distance distribution that's expected, hence when the decay stops/increase starts, that's the start of non-expected long range interactions
...
def negative_generating(M, kde, positives, lower, long_start, long_end):
    ...
    N = 4 * len(positives)
    # part 1: kde (sample from the pre-trained positive distribution)
    part1 = kde.resample(N).astype(int).ravel()
    part1 = part1[(part1 >= lower) & (part1 <= long_end)]

    # part 2: random long-range interactions from M
    part2 = []
