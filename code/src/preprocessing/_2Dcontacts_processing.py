...
def csr_contact_matrix(
    norm, hicfile, chr1loc, chr2loc, unit, binsize, is_synapse=False
):
    """
...
def straw_contact_matrix(
    ...
    mtx = hic.getMatrixZoomData(
        chrom1, chrom2, data_type, normalization, "BP", resolution
    )  # MatrixZoomData object

    numpy_matrix = mtx.getRecordsAsMatrix(
        gr1, gr2, gc1, gc2
    )  # dense matrix loading of a specific genomic region
    hic.close()
