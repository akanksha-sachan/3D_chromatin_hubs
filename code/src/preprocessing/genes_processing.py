...
def fetch_gene_ensemblAPI(ensembl_ids, batch_size=1000, ref_genome="hg38"):
    ...
    for i in tqdm(range(0, len(ensembl_ids), batch_size)):
        ...
        if response.ok:
            ...
            print(
                f"API Request Failed: {response.status_code} {response.reason} for batch starting at index {i}"
            )
...
def fetch_gene_gtf(gtf_file, ensembl_ids, ref_genome="hg38"):
    ...
    Returns:
        pd df: A DataFrame containing 'gene_id', 'gene_type', 'chrom', 'Txstart', 'Txend', 'gene_name' for each gene
    """
    ...
    df = gtf_genes[
        ["gene_id", "gene_type", "gene_name", "chrom", "Txstart", "Txend"]
    ].copy()
...
def processRNAseqData(
    infile, outfile, gtf_file, threshold=0.1, normalisation="log1p_t", ref_genome="hg38"
):
    ...
    Returns:
        DataFrame: A DataFrame containing gene names and expressed gene expression values
    """
    ...
    expressed_genes = pd.merge(
        rna_data, gene_df, left_on="gene_id", right_on="gene_id", how="inner"
    )
...
def assignBins2Genes(infile, bins_files, ref_genome="hg38"):
    """
    ...
    Returns:
        pd df column headers: gene_name, TSSBin_10kb: chrN:start, TSSBin_{other}, BodyBins_10kb: tuple of chrN:starts, BodyBins_{other}: tuple
    """
    ...
    gene_chrom_bin_num = genes[
        ["gene_name", "chrom", "Txstart", "Txend", "normalized_expression"]
    ].copy()

    for res0, bins_file in bins_files.items():
        # load the bins file
        bins = pd.read_csv(
            bins_file, sep="\t", header=None, names=["chrom", "start", "end"]
        )
        ...
        for chrom, group in bins.groupby("chrom"):
            ...
            chrom_intervals[chrom] = pd.IntervalIndex.from_arrays(
            ...
            if row["chrom"] in chrom_intervals:
                ...
                try:
                    ...
                    if not preceding_intervals.empty:
                        interval = preceding_intervals[
                            -1
                        ]  # Last interval before or at TSS
                        return f"{row['chrom']}:{interval.left}"
            return None

        gene_chrom_bin_num[f"TSSBin_{res0}"] = gene_chrom_bin_num.apply(
            getTSSBin, axis=1
        )
        ...
        def getBodyBins(row):
            if row["chrom"] in chrom_intervals:
                intervals = chrom_intervals[row["chrom"]]
                overlapping_intervals = intervals.overlaps(
                    pd.Interval(row["Txstart"], row["Txend"], closed="right")
                )
                return tuple(
                    f"{row['chrom']}:{intv.left}"
                    for intv in intervals[overlapping_intervals]
                )
            return ()

        gene_chrom_bin_num[f"BodyBins_{res0}"] = gene_chrom_bin_num.apply(
            getBodyBins, axis=1
        )

    # save the bin location for each gene to a file
    gene_chrom_bin_num.to_csv(
        f"/Users/Akanksha/MaGroup/Genomic Hubs/workflow/tmp/gene_chrom_bin_num_{ref_genome}_gm12878.txt",
        sep="\t",
        index=False,
    )
    return gene_chrom_bin_num
...
def assignTSS2HiCBinID(infile, res0, bins_3D, ref_genome="hg38"):
    ...
    gene_bins["start"] = (
        gene_bins["start"].astype(str).str.replace(",", "").astype(int)
    ...
    gene_to_bin_dict = dict(
        zip(
            merged_df.loc[
                ~merged_df["gene_name"].isin(non_matched_genes["gene_name"]),
                "gene_name",
            ],
            merged_df.loc[
                ~merged_df["gene_name"].isin(non_matched_genes["gene_name"]), "bin_id"
            ],
        )
    )
...
if __name__ == "__main__":
    # loading data
    cool_file = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/data/GM12878/4DNFITRVKRPA_GM12878.mcool"
    resolution = 10000
    c = cooler.Cooler(cool_file + "::/resolutions/" + str(resolution))
    bins = c.bins()[:]  # whole genome assignment of genes to bins
    res_str = "10kb"
    infile = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/data/GM12878/gene_chrom_bin_num_hg19_gm12878.txt"
    result, non_matched = assignTSS2HiCBinID(infile, res_str, bins)
    result_df = pd.DataFrame(list(result.items()), columns=["Gene_Name", "Bin_ID"])
    result_df["Bin_ID"] = result_df["Bin_ID"].astype(int)
    # save to file
    result_df.to_csv(
        "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/tmp/genes_binID_hg19_gm12878.txt",
        sep="\t",
    )

    ####testing RNA-seq data processing
    infile = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/data/GM12878/ENCODE_hg38/RNA_ENCSR820PHH/ENCFF345SHY_gene_quant.tsv"
    outfile = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/tmp/expressed_genes_hg38_gm12878.txt"
    gtf_file = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/data/gencode.v45.basic.annotation.hg38.gtf"
    rna_data = processRNAseqData(infile, outfile, gtf_file)

    ####testing gene assignment to bins
    infile = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/tmp/expressed_genes_hg38_gm12878.txt"
    # testing gene assignment to bins
    bins_files = {
        "10kb": "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/tmp/10000/bins_hg38.bed"
    }
    infile = "/Users/Akanksha/MaGroup/Genomic Hubs/workflow/tmp/expressed_genes_hg38_gm12878.txt"
    assignBins2Genes(infile, bins_files)
